...
function Sidebar() {
  ...
  return (
    <div className="flex">
      {mobileOpen && (
        ...
        >
          <button
            ...
            } top-9 w-7 border-gray-900 rounded-full ${!open && "rotate-180"}`}
          ...
          >
            {open && (
              <div className="flex items-center gap-4">
                <img
                  src={profile}
                  alt="User"
                  className="h-8 w-8 rounded-full select-none ml-1"
                />
            ...
            <Link
              ...
              className={`flex items-center ${
                open ? "p-2" : "p-1"
              } cursor-pointer text-gray-300 text-sm rounded-full`}
            >
              <IoMdLogOut
                size={22}
      ...
      <button
        className="sm:flex lg:hidden md:hidden p-2 text-gray-100 bg-gray-900 fixed top-5 right-5 rounded-md z-20"
        onClick={handleMobileToggle}
      >
        {mobileOpen ? <IoMdClose size={24} /> : <RxHamburgerMenu size={24} />}
      </button>
    </div>
  );
}
